...
export const StreamsList = memo(({ streams: initialStreams, onStreamClick }: StreamsListProps) => {
  ...
  return (
    <div ref={containerRef} className="h-full w-full pt-16 pb-20 px-3 md:px-4 relative overflow-y-auto overscroll-y-contain">
      {pullDistance > 0 && (
        <div 
          className="absolute top-16 left-0 right-0 flex items-center justify-center z-30 transition-opacity"
          style={{ 
            opacity: pullProgress,
            transform: `translateY(${pullDistance - 80}px)`
          }}
        >
          <div className="bg-card/90 backdrop-blur-xl border border-primary/30 rounded-full p-2.5 shadow-lg">
            {isRefreshing ? (
              ...
              <svg 
                className={`w-5 h-5 text-primary transition-transform ${pullProgress >= 1 ? 'rotate-180' : ''}`}
      ...
      <div className="max-w-4xl mx-auto">
        <h2 className="text-xl md:text-2xl font-bold text-white mb-4 md:mb-6 flex items-center gap-2 px-1">
